...
using TeleChat.Api.Hub;

namespace TeleChat.Api.Repositories.App;
...
public class AppRepository(IHubContext<ChatHub, IChatHub> hubContext, DBContext context) : IAppRepository
{
    ...
    {
        var messageReaction = await _context.MessageReaction.ToListAsync();
        var messages = await _context.Message.Where(x => x.GroupChatId == chatId).ToListAsync();
        var reactions = await _context.Reaction.ToListAsync();

        var result = 
            from msgReaction in messageReaction
            join msg in messages on msgReaction.MessageId equals msg.Id
            join rct in reactions on msgReaction.ReactionId equals rct.Id
            select new ReactionDto
            {
                MessageReactionId = msgReaction.Id,
                ReactionId = msgReaction.ReactionId,
                Value = rct.Value,
                UserId = msg.UserId,
                MessageId = msg.Id
            };
